...
bool FLudeoWritableObject::BindPlayer(const FLudeoPlayer& Player) const
{
	const FLudeoResult Result = ConditionalRoomWriterSetCurrent(RoomWriterHandle);

	if (Result.IsSuccessful())
	{
		return (ludeo_DataWriter_SetPlayerBinding(TCHAR_TO_UTF8(Player.GetPlayerID().GetCharArray().GetData())) == LUDEO_TRUE);
	}
